...
// Renders the full post-detail layout (header, image carousel, engagement row,
// caption/hashtags, comments sheet, menu, collection picker) for a single post.
// Used by PostDetailScreen (single post) and CollectionPostsScreen (vertical list).
export default function PostDetailCard({
  ...
  const openAnalytics = () => {
    ...
    analyticsService.getBookmarkCounts([post.id]).then((counts) => {
      setSavesCount(counts[post.id] || 0);
  ...
  const handleRemoveFromCollection = () => {
    setMenuVisible(false);
    onRemoveFromCollection?.(post.id);
  };
  ...
  const handleDelete = () => {
    ...
    const doDelete = async () => {
      ...
      else if (onDeleted) onDeleted(post.id);
      else goBack();
    };
    if (Platform.OS === 'web') {
    ...
    <>
      ...
      <View style={[styles.postCard, flush && styles.postCardFlush, { backgroundColor: colors.surface }]}>
        ...
        <View style={styles.contentArea}>
          ...
          <View style={styles.titleRow}>
            ...
            {stylistName && (
              <TouchableOpacity
                ...
                activeOpacity={0.7}
              >
                <Scissors size={13} color="#B35D2B" strokeWidth={1.5} />
          ...
          {/* Owner-only: link to this post's analytics */}
          {isOwnPost && (
            <TouchableOpacity style={styles.analyticsLink} onPress={openAnalytics} activeOpacity={0.7}>
              <Ionicons name="bar-chart-outline" size={15} color={colors.textSecondary} />
              <Text style={[styles.analyticsLinkText, { color: colors.textSecondary }]}>View Analytics</Text>
            </TouchableOpacity>
          )}
        </View>
      </View>
        ...
        <Pressable style={[styles.menuOverlay, { backgroundColor: colors.overlay }]} onPress={() => setMenuVisible(false)}>
          <View style={[styles.menuSheet, { backgroundColor: colors.surface }]}>
            ...
            {onRemoveFromCollection && (
              <TouchableOpacity style={[styles.menuItem, { borderBottomColor: colors.borderLight }]} onPress={handleRemoveFromCollection}>
                <Ionicons name="bookmark-outline" size={22} color={colors.text} />
                <Text style={[styles.menuItemText, { color: colors.text }]}>Remove from Collection</Text>
              </TouchableOpacity>
            )}
            {isOwnPost ? (
      ...
      {/* ── Post analytics modal (owner-only) ── */}
      <Modal visible={analyticsVisible} transparent animationType="fade" onRequestClose={() => setAnalyticsVisible(false)}>
        <Pressable style={[styles.menuOverlay, { backgroundColor: colors.overlay }]} onPress={() => setAnalyticsVisible(false)}>
          <Pressable style={[styles.analyticsSheet, { backgroundColor: colors.surface }]} onPress={() => {}}>
            <View style={[styles.menuHandle, { backgroundColor: colors.border }]} />
            <Text style={[styles.analyticsTitle, { color: colors.text }]}>Post Analytics</Text>

            <View style={styles.analyticsStatsRow}>
              <View style={styles.analyticsStatItem}>
                <Ionicons name="heart" size={20} color="#F27C7C" />
                <Text style={[styles.analyticsStatValue, { color: colors.text }]}>{likesCount}</Text>
                <Text style={[styles.analyticsStatLabel, { color: colors.textSecondary }]}>Likes</Text>
              </View>
              <View style={styles.analyticsStatItem}>
                <Ionicons name="chatbubble" size={20} color={colors.text} />
                <Text style={[styles.analyticsStatValue, { color: colors.text }]}>{comments.length || dbCommentCount}</Text>
                <Text style={[styles.analyticsStatLabel, { color: colors.textSecondary }]}>Comments</Text>
              </View>
              <View style={styles.analyticsStatItem}>
                <Ionicons name="bookmark" size={20} color={OCHRE} />
                {savesLoading ? (
                  <ActivityIndicator size="small" color={colors.textSecondary} style={{ marginVertical: 2 }} />
                ) : (
                  <Text style={[styles.analyticsStatValue, { color: colors.text }]}>{savesCount}</Text>
                )}
                <Text style={[styles.analyticsStatLabel, { color: colors.textSecondary }]}>Saves</Text>
              </View>
            </View>
            ...
            >
              <Text style={[styles.menuCancelText, { color: colors.text }]}>Close</Text>
            </TouchableOpacity>
          </Pressable>
        </Pressable>
      </Modal>
...
const makeStyles = (c) => StyleSheet.create({
  ...
  stylistPillText: { fontSize: 12, fontFamily: 'Figtree_600SemiBold', color: '#B35D2B' },
  ...
  analyticsLink: { flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 10 },
  analyticsLinkText: { fontSize: 13, fontFamily: 'Figtree_600SemiBold' },
  ...
  analyticsTitle: { fontSize: 17, fontFamily: 'Figtree_700Bold', textAlign: 'center', marginBottom: 20 },
  analyticsStatsRow: { flexDirection: 'row', justifyContent: 'space-around', paddingHorizontal: 24, marginBottom: 8 },
  analyticsStatItem: { alignItems: 'center', gap: 4, minWidth: 64 },
  analyticsStatValue: { fontSize: 20, fontFamily: 'Figtree_700Bold' },
  analyticsStatLabel: { fontSize: 12, fontFamily: 'Figtree_500Medium' },
